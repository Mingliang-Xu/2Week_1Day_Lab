...
sum = console.log(add(Number("10"), Number(23)));
...
console.log(exclaimThree("asdfdsa"));
...
const nameGreeting = nameCheck("Oliver");
...
const bestMovie = aMovie => console.log(`${aMovie} is the best movie ever!`);
...
const capLetters = anyString => console.log(anyString.toUpperCase());

capLetters(sampleString);
...
function buyFrogs (myGold) 

{
  let countOfFrogs = Math.floor(myGold / 3);
  return countOfFrogs;
  
}

let totalFrogs = buyFrogs(24);
...
function sortedArr (array){
  ...
  for ( i = 0; i < array.length ; i++){
    if (array[i] > array[i+1]){
      return false; // if this is true, return will end the function
  }
  }
  return true; // after the for loop, there is no false result
}

let arrayIsAscending = sortedArr(sampleArray);
